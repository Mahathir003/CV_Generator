...
const sampleMockData = {
  professional: {
    ...
    subtitle: "Baba Yaga",
    about:
      "Highly disciplined and resourceful operations specialist with extensive experience in high-risk environments. Known for strategic planning, precision execution, problem-solving under pressure, and maintaining confidentiality in sensitive assignments.",
    phone: "+123-456-7890",
    email: "babayaga@reallygreatsite.com",
...
function selectTemplate(templateId) {
  ...
  if (!userModifiedColors) {
    const profileColors = templateColorProfiles[currentPurpose][templateId];
    if (profileColors) {
      document.getElementById("primaryColor").value = profileColors.primary;
      document.getElementById("secondaryColor").value = profileColors.secondary;
...
function switchPurpose(purpose) {
  ...
  if (purpose === "professional") {
    document.getElementById("purpose-prof").classList.add("purpose-btn-active");
  ...
  if (!userModifiedColors) {
    const profileColors = templateColorProfiles[purpose][currentTemplateId];
    if (profileColors) {
      document.getElementById("primaryColor").value = profileColors.primary;
      document.getElementById("secondaryColor").value = profileColors.secondary;
      updateCanvasColors(false);
            ...
            <div style="display: block; font-family: ${fontName};">
                ...
                <div style="display: block; font-size: 11px; color: #e2e8f0;">
                    ${data.languages
                      ...
                      .map(
                        (lang) =>
                          `<div style="margin-bottom: 4px;">• ${lang}</div>`,
                      )
                      .join("")}
                </div>
            </div>`
